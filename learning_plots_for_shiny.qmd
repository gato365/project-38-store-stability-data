...
mod_1_mongo_df <- mongo_df %>%
  mutate(specificTime = ymd_hms(specificTime), 
         specificDate = date(specificTime)) %>% 
  filter(specificDate >= first_day & specificDate <= last_day) %>% 
  mutate(timeBlock = factor(timeBlock, levels = c("4 am", "7:45 am", "10:45 am", "12 pm", "3 pm", "6 pm", "8:30 pm")),
         timeBlock = fct_relevel(timeBlock, c("4 am", "7:45 am", "10:45 am", "12 pm", "3 pm", "6 pm", "8:30 pm")))
...
g_mongo_df <- mod_1_mongo_df %>% 
  select(specificDate,timeBlock,goalOutcome) %>% 
  arrange(specificDate,timeBlock) %>% 
  pivot_wider(names_from = timeBlock, values_from = goalOutcome) %>% 
  select(-specificDate)
...
```{r}
mongo_df %>% 
  select(specificTime,timeBlock,foodQuality)
```
